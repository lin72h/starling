...
private enum TermTheme {
    static let background = Color(0xD91E2127)
    static let defaultFg: UInt32 = 0xFFD4D4D4
    static let cursorBg: UInt32 = 0xFFD4D4D4
    static let cursorFg: UInt32 = 0xFF1E2127
    static let fontSize: Double = 13
}

// MARK: - Monospace font registration
...
class _TerminalAppState: State<StatefulWidget>, @unchecked Sendable {
    ...
    private func _handleKey(_ keyData: KeyData) -> Bool {
        ...
        if _exited {
            if keyData.logical == 0xFF0D {  // Enter — restart the shell
                _lock.lock()
                let (cols, rows) = (emulator.cols, emulator.rows)
            ...
            for r in 0 ..< grid.count {
                ...
                layers.append(Positioned(
                    ...
                    child: SizedBox(
                        ...
                        height: cellH,
                        child: DecoratedBox(
                            decoration: BoxDecoration(color: Color(0x4066AAFF))
                        )
                    )
                ))
            }
            ...
            layers.append(
                Positioned(
                    ...
                    child: SizedBox(
                        ...
                        height: cellH,
                        child: DecoratedBox(
                            decoration: BoxDecoration(color: Color(0x99D4D4D4))
                        )
                    )
                )
            )
    ...
    private func _rowWidget(_ line: [TermCell]) -> Widget {
        ...
        for c in 0 ..< headEnd {
            ...
            if cell.attrs.contains(.reverse) {
                ...
                let bg = cell.fg == 0 ? TermTheme.defaultFg : cell.fg
                cell.fg = fg
                cell.bg = bg
