...
# The 3D desktop's worlds for the Filament renderer, one directory each
# (a glTF, a cmgen'd sky, world.json), generated by build/tools/*-world.py
# and checked in; the shell finds them at share/starling/worlds/<name>.
for w in "$REPO"/shell/Resources/Worlds/*/; do
    [ -d "$w" ] || continue
    mkdir -p "$SHARE/worlds/$(basename "$w")"
    for f in "$w"*; do
        case "$f" in *.md) continue ;; esac
        install -m644 "$f" "$SHARE/worlds/$(basename "$w")/"
    done
done
