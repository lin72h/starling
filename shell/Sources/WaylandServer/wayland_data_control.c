...
/* The primary selection (v2): the same, from server->primary. */
static void dc_send_primary_to_device(struct WaylandServer* server,
                                      struct wl_resource* device) {
    if (wl_resource_get_version(device) <
        ZWLR_DATA_CONTROL_DEVICE_V1_PRIMARY_SELECTION_SINCE_VERSION)
        return;
    if (!server->primary.owner) {
        zwlr_data_control_device_v1_send_primary_selection(device, NULL);
        return;
    }
    struct WaylandDataControlOffer* offer = calloc(1, sizeof(*offer));
    if (!offer) return;
    ...
    for (int i = 0; i < server->primary.mime_count; i++)
        zwlr_data_control_offer_v1_send_offer(offer_res, server->primary.mimes[i]);
    zwlr_data_control_device_v1_send_primary_selection(device, offer_res);
}
...
void wayland_data_control_broadcast_primary(struct WaylandServer* server) {
    ...
    wl_list_for_each(dcd, &server->data_control_devices, link)
        dc_send_primary_to_device(server, dcd->resource);
}
...
static void dc_device_set_primary_selection(struct wl_client* client,
    ...
    if (!dev) return;
    if (source_resource) {
        struct WaylandDataControlSource* s = wl_resource_get_user_data(source_resource);
        if (!s) return;
        s->used = 1;
...
static void dc_manager_get_data_device(struct wl_client* client,
    ...
    dc_send_primary_to_device(server, device);
}
